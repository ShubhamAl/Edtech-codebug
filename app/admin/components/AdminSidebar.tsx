      ...
      `}>
        ...
        <div className="flex items-center justify-between mb-12 px-2 shrink-0">
          <div className="flex items-center gap-4 group cursor-default">
            <div className={`relative h-12 w-12 bg-violet-500 ${blackBorder} rounded-2xl flex items-center justify-center rotate-3 group-hover:rotate-0 transition-all duration-300 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]`}>
              <GraduationCap className="text-white w-7 h-7" strokeWidth={3} />
            </div>
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-black text-black/40 dark:text-white/40 tracking-[0.3em] uppercase mt-1">Admin Console</span>
            </div>
          </div>
        ...
        {/* ROLE BADGE */}
        <div className={`mb-8 mx-2 flex items-center gap-3 bg-white dark:bg-zinc-900 border-2 border-black dark:border-white rounded-xl px-4 py-2.5 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] dark:shadow-[3px_3px_0px_0px_rgba(255,255,255,1)]`}>
          <ShieldCheck size={16} className="text-violet-500" strokeWidth={3} />
          <span className="text-[10px] font-black text-black dark:text-white uppercase tracking-[0.2em]">Master Admin</span>
        </div>
            ...
            return (
              <Link key={item.name} href={item.href} onClick={() => setIsOpen(false)}>
                <div className={`
                  ...
                  ${isActive
                    ? `bg-violet-500 ${blackBorder} shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] translate-x-1`
                    : `hover:bg-white dark:hover:bg-zinc-900 border-2 border-transparent`}
                `}>
                  <div className="flex items-center gap-4 relative z-10">
                    <div className={`
                      transition-all duration-200
                      ${isActive ? "text-white scale-110" : "text-black/40 dark:text-white/40 group-hover:text-black dark:group-hover:text-white"}
                    `}>
                    ...
                    <span className={`text-xs font-black uppercase tracking-widest transition-colors
                      ${isActive ? "text-white" : "text-black/40 dark:text-white/40 group-hover:text-black dark:group-hover:text-white"}`}>
                      {item.name}
                    </span>
                  </div>

                  {isActive && (
                    <motion.div layoutId="adminSidebarZap" className="relative z-10">
                      <Zap className="w-4 h-4 text-white fill-white" strokeWidth={3} />
                    </motion.div>
                  )}
                </div>
              </Link>
        ...
        <div className={`mt-8 mb-8 p-6 bg-white dark:bg-zinc-900 rounded-[2rem] ${blackBorder} relative overflow-hidden group shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]`}>
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            <span className="text-[11px] font-black text-black dark:text-white uppercase tracking-tighter leading-none">Access Level</span>
            <span className={`text-[10px] font-black text-white bg-violet-500 px-2 py-1 rounded-md border-2 border-black`}>ROOT</span>
          </div>
          ...
          <div className={`h-3 w-full bg-[#F9F4F1] dark:bg-zinc-800 rounded-full border-2 border-black dark:border-white overflow-hidden p-[2px]`}>
            <motion.div
              ...
              className="h-full bg-violet-500 rounded-full border-r-2 border-black dark:border-white"
            />
          </div>
